...
public class DarkPawn extends ChessPiece {
    DarkPawn(Context context, int id, int y, int x) {
    ...
    public boolean validCapture(int row, int column, int turnNumber)
    {
        ...
        {
            if(super.getGame().getPiecesL().contains(super.getGame().getBoard()[row - 1][column]))
            {
                if (turnNumber - 1 == super.getGame().getBoard()[row - 1][column].getLastMove()) {
                    super.getGame().getPiecesL().remove(super.getGame().getBoard()[row - 1][column]);
    ...
    public boolean validMove(int row, int column, int turnNumber)
    {
        ...
        if (super.isMoveFirst())
        {
            ...
            else if ((row - super.getRow()) == 1 || (row - super.getRow()) == 2)
            {
        ...
        else
        {
            if (validCapture(row, column, turnNumber))
            {
                if(row == 7)
                {
                    ...
                    activity.showDialog();
                    super.getGame().setPromoteDark(this);
                }
                return true;
            }
            if(super.getColumn() != column)
